...
class SidebarController
{
    public function __invoke(
        ...
        if ($customer !== null) {
            $this->applyLinks($config, $context, $customer, $sidebar, $logger);
        }
    ...
    /**
     * Apply the configured header links, unless the builder set its own.
     *
     * Doing this here rather than in the builder means the icons work for an
     * installation that has only edited the config file, which is the common
     * case. A builder that calls links() itself is left alone.
     */
    protected function applyLinks(
        ConfigRepository $config,
        ...
        $configured = (array) $config->get('helpscout-sidebar.links', []);

        if ($configured === []) {
            return;
        }

        $sidebar->links(SidebarLinks::resolve(
            $configured,
            $customer,
            $context,
            $logger,
            SidebarLinks::baseFrom($config),
        ));
    }
